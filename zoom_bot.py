...
async def run_bot(playwright: Playwright, bot_id: int, semaphore: asyncio.Semaphore):
    ...
    async with semaphore:
        ...
        try:
            # All the login flow remains the same...
            ...
            # --- CRITICAL CHANGE: GRACEFUL AUDIO JOIN ---
            # We will now TRY to join audio, but we will not fail if we can't.
            try:
                print(f"[{bot_name}] Looking for 'Join Audio by Computer' button...")
                join_audio_button = page.get_by_role("button", name="Join Audio by Computer")
                await join_audio_button.wait_for(timeout=30000) # Wait a shorter time
                await join_audio_button.click()
                print(f"[{bot_name}] Successfully connected audio.")
            except Exception:
                # THIS IS THE KEY: We catch the error, log a simple message, and MOVE ON.
                print(f"[{bot_name}] Could not connect audio, but assuming successful entry.")

            # Regardless of the audio step, we now start the keep-alive routine.
            await keep_alive_in_meeting(page, bot_name)

        except Exception as e:
            # This 'except' block will now only catch critical errors during the login flow.
